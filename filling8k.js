    ...
    for (const link of data) {
        ...
        const filingData = {
            ...
            jsonContent: itemsContent.reduce((acc, item) => {
                const name = item.itemname.split("-");
                const concatname = name[0] + "_" + name[1];

                if(item.content.topic != "" && item.content.content.length > 0) {
                    acc[`item_${concatname}`] = item.content;
                }
                return acc;
            }, {})
        };
        ...
        fs.writeFileSync("itemfilling8k_final.json", JSON.stringify(finalContent, null, 2));
    }
